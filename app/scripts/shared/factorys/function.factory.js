...
function FunctionFactory() {
  ...
  function redirect_to(location) {
    window.location.assign(window.location.origin + window.location.pathname + "#/" + location);
  }

  function find_duplicate_values (arr, prop, len=0, find=[]) {
    // console.log(arr, len, find)
    if(len == arr.length){
      return find
    }
    else{
      let item = arr[len][prop]
      for (let i = 0; i < arr.length; i++) {
        if(i!=len && item == arr[i][prop] && find.indexOf(item) ==-1){
           find.push(item);
        }
      };
      return find_duplicate_values(arr, prop, ++len, find)
    }
  }
